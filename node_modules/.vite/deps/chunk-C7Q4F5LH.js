...
var BaseError2 = class extends Error {
  constructor(shortMessage, args = {}) {
    ...
    const docsPath6 = args.cause instanceof BaseError2 ? args.cause.docsPath || args.docsPath : args.docsPath;
    const message = [
...
async function estimateGas(client, args) {
  ...
  try {
    ...
    assertRequest(args);
    const format3 = ((_c = (_b = (_a = client.chain) == null ? void 0 : _a.formatters) == null ? void 0 : _b.transactionRequest) == null ? void 0 : _c.format) || formatTransactionRequest;
    const request = format3({
      // Pick out extra data that might exist on the chain's transaction request type.
      ...extract(rest, { format: format3 }),
...
async function sendTransaction(client, args) {
  ...
  try {
    ...
    const format3 = ((_c = (_b = chain == null ? void 0 : chain.formatters) == null ? void 0 : _b.transactionRequest) == null ? void 0 : _c.format) || formatTransactionRequest;
    const request = format3({
      // Pick out extra data that might exist on the chain's transaction request type.
      ...extract(rest, { format: format3 }),
      accessList,
...
function createClient({ account, batch, chain, key = "base", name = "Base Client", pollingInterval = 4e3, transport, type = "base" }) {
...
async function recoverPublicKey({ hash, signature }) {
  ...
  if (v === 0 || v === 1)
    v += 27;
  const { secp256k1 } = await import("./secp256k1-KKGTYJBJ.js");
...
async function getEnsAddress(client, { blockNumber, blockTag, name, universalResolverAddress: universalResolverAddress_ }) {
  ...
  try {
    ...
    if (err instanceof ContractFunctionExecutionError) {
      const reason = (_a = err.cause) == null ? void 0 : _a.reason;
      if (reason == null ? void 0 : reason.includes("Wildcard on non-extended resolvers is not supported"))
        return null;
    }
    throw err;
  }
...
function createTestClient({ account, chain, key = "test", name = "Test Client", mode, pollingInterval, transport }) {
  return createClient({
    ...
    transport,
    type: "testClient"
  }).extend(() => ({ mode })).extend(testActions({ mode }));
}
...
//# sourceMappingURL=chunk-C7Q4F5LH.js.map
